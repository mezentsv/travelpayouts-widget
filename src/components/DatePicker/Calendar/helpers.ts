...
export const getNumOfDaysInGivenMonth = (month = THIS_MONTH, year = THIS_YEAR) => {
...
export const isDate = (date?: Date) => {
  ...
  return isDateObj && Boolean(isValidDate);
};
...
export const getDateISO = (date = new Date()) => {
  if (!isDate(date)) {
    throw new Error("Invalid date format. Please follow 'ISO 8601' format; 2013-08-30");
  }
...
export default (month = THIS_MONTH, year = THIS_YEAR) => {
  ...
  const thisMonthDates = [...new Array(monthDays)].map((n, index) => [year, zeroPad(month, 2), zeroPad(index + 1, 2)]);
